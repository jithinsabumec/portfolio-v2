...
function ToggleGroup({
  ...
  return (
    <ToggleGroupPrimitive
      ...
      data-size={size}
      className={cn(
        'group/toggle-group flex gap-0.5 w-fit items-center rounded-xl data-[variant=outline]:shadow-xs data-[variant=outline]:border data-[variant=outline]:p-0.5',
        className,
      )}
      {...props}
    >
      <ToggleGroupProvider value={{ variant, size }}>
        {props.type === 'single' ? (
          <ToggleGroupHighlightPrimitive className="bg-[#E6E6E6] rounded-xl">
            {children}
...
function ToggleGroupItem({
  ...
  return (
    <ToggleGroupHighlightItemPrimitive
      value={props.value}
      className={cn(type === 'multiple' && 'bg-[#E6E6E6] rounded-xl')}
    >
      <ToggleGroupItemPrimitive
        ...
        className={cn(
          ...
          'min-w-0 border-0 flex-1 shrink-0 shadow-none rounded-xl focus:z-10 focus-visible:z-10',
          className,
        )}
        {...props}
      >
